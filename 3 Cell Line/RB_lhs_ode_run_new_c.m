function CalibrationOutput = RB_lhs_ode_run_new_c(odeSettings, runIteration)

% Changes to the lhs_ode_run.m file were made to make this file more
% compatible with the calibration program.  It is now named
% lhs_ode_run_new_c.m  Main changes include an output for the function that
% contains the model run results packaged up into a structure called
% Calibration Output and some changes to the cleanup function.
%
% -------------------------------------------------------------------------
%
...
totalPrccSignEval = []; %assignin('base', 'totalPrccSignEval',totalPrccSignEval);
...
CalibrationOutput = {}; %assignin('base', 'CalibrationOutput', CalibrationOutput);
...
fprintf('\n=====================================\n');
fprintf('   ODE LHS Run %d', runIteration);
fprintf('\n=====================================\n');

% Put current value in base workspace so the cleanup function can save it.
%assignin('base','runIteration', runIteration);
...
BATs_numbers=table2array(readtable('BATs_numbers_C.xlsx','Sheet','MCF7','VariableNamingRule','preserve'));
            ...
            NBAT_fit = BATs_numbers(:,2);
            LBAT_fit = BATs_numbers(:,4);
            PBAT_fit = BATs_numbers(:,3);
            DBAT_fit = BATs_numbers(:,5);
            N1 = csaps(cell_prog_time,NBAT_fit,.05);
            L1 = csaps(cell_prog_time,LBAT_fit,.03);
            P1 = csaps(cell_prog_time,PBAT_fit,.05);
            D1 = csaps(cell_prog_time,DBAT_fit,.05);
            MN2 = fnder(N1);
            ML2 = fnder(L1);
            MP2 = fnder(P1);
            MD2 = fnder(D1);
...
BATs_numbers=table2array(readtable('BATs_numbers_C.xlsx','Sheet','CAMA1','VariableNamingRule','preserve'));
            ...
            DBAT_fit = BATs_numbers(:,5);
            N1 = csaps(cell_prog_time,NBAT_fit,.05);
            L1 = csaps(cell_prog_time,LBAT_fit,.05);
            P1 = csaps(cell_prog_time,PBAT_fit,.05);
            D1 = csaps(cell_prog_time,DBAT_fit,.05);
            CN2 = fnder(N1);
            CL2 = fnder(L1);
...
for run = 1:odeSettings.NR
    ...
    try
        if ~isfield(odeSettings, 'pulseTimePoints')
            ...
            [t,y] = ode15s(@(t,y)odeSettings.odeModelHandle(t, y, runParameters,MN2,ML2,MP2,MD2,CN2,CL2,CP2,CD2,TN2,TL2,TP2,TD2), odeSettings.tspan, y0, options);
            ...
            lastwarn('') % Clear last warning message
            [t,y] = ode15s(@(t,y)odeSettings.odeModelHandle(t, y, runParameters,MN2,ML2,MP2,MD2,CN2,CL2,CP2,CD2,TN2,TL2,TP2,TD2), preTspan, y0, options);
            [savedWarnMsg, savedWarnId] = checkWarning();
            ...
            if (length(t) == length(preTspan))
                ...
                for i = 1:length(odeSettings.pulseTimePointIdx)
                    ...
                    lastwarn('') % Clear last warning message
                    [tCurr,yCurr] = ode15s(@(tCurr,yCurr)odeSettings.odeModelHandle(tCurr, yCurr, runParameters,MN2,ML2,MP2,MD2,CN2,CL2,CP2,CD2,TN2,TL2,TP2,TD2), currTspan, y0, options);
                    [savedWarnMsg, savedWarnId] = checkWarning();
...
% Put current value in base workspace so the cleanup function can save it.
%assignin('base', 'odeOut', odeOut);
CalibrationOutput.odeOut = odeOut;
CalibrationOutput.paramMatrix = paramMatrix;
CalibrationOutput.LHSmatrix = LHSmatrix;
CalibrationOutput.icMatrix = icMatrix;
CalibrationOutput.PRCC_var = PRCC_var;
CalibrationOutput.odeSettings = odeSettings;
CalibrationOutput.pulseMatrix = pulseMatrix;
...
% runIteration = evalin('base', 'runIteration');
% 
...
% CalibrationOutput.odeSettings = odeSettings;
% CalibrationOutput.pulseMatrix = pulseMatrix;
% %assignin('base', 'CalibrationOutput', CalibrationOutput);
end
