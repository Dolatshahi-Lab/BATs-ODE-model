...
BATs_numbers=table2array(readtable('BATs_numbers_D.xlsx','Sheet','Donor_1','VariableNamingRule','preserve'));
            ...
            N1 = csaps(cell_prog_time,NBAT_fit,0.2,[],[1 0.04 .1 10 10 10]);
            L1 = csaps(cell_prog_time,LBAT_fit,0.05);
            P1 = csaps(cell_prog_time,PBAT_fit,0.2,[],[1 0.04 .1 10 10 10]);
            D1 = csaps(cell_prog_time,DBAT_fit,0.05);
...
for run = 1:odeSettings.NR
    ...
    try
        if ~isfield(odeSettings, 'pulseTimePoints')
            ...
            [t,y] = ode15s(@(t,y)odeSettings.odeModelHandle(t, y, runParameters,NC_2,LC_2,PC_2,DC_2,ND_2,LD_2,PD_2,DD_2), odeSettings.tspan, y0, options);
            ...
            lastwarn('') % Clear last warning message
            [t,y] = ode15s(@(t,y)odeSettings.odeModelHandle(t, y, runParameters,NC_2,LC_2,PC_2,DC_2,ND_2,LD_2,PD_2,DD_2), preTspan, y0, options);
            [savedWarnMsg, savedWarnId] = checkWarning();
            ...
            if (length(t) == length(preTspan))
                ...
                for i = 1:length(odeSettings.pulseTimePointIdx)
                    ...
                    lastwarn('') % Clear last warning message
                    [tCurr,yCurr] = ode15s(@(tCurr,yCurr)odeSettings.odeModelHandle(tCurr, yCurr, runParameters,NC_2,LC_2,PC_2,DC_2,ND_2,LD_2,PD_2,DD_2), currTspan, y0, options);
                    [savedWarnMsg, savedWarnId] = checkWarning();
